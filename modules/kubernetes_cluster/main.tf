...
resource "azurerm_kubernetes_cluster" "aks" {
  ...
  //node_resource_group     = format("vmss-%s",data.azurerm_resource_group.kubernetes.name)
...
/*
  addon_profile {
    ...
    oms_agent {
      ...
      log_analytics_workspace_id = var.enable_log_analytics_workspace ? azurerm_log_analytics_workspace.log_workspace[0].id : null
    }
...
resource "azurerm_log_analytics_solution" "log_solution" {
  ...
  location              = data.azurerm_resource_group.kubernetes.location
  resource_group_name   = var.resource_group_name
  workspace_resource_id = azurerm_log_analytics_workspace.log_workspace[0].id
  workspace_name        = azurerm_log_analytics_workspace.log_workspace[0].name
