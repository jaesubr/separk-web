...
resource "azurerm_network_security_rule" "predefined_rules" {
  ...
  name                                       = format("%s%s%s",element(var.rules[lookup(var.predefined_rules[count.index], "name")], 1),lookup(var.predefined_rules[count.index], "name"),element(var.rules[lookup(var.predefined_rules[count.index], "name")], 0))
...
resource "azurerm_network_security_rule" "custom_rules" {
  ...
  name                                       = format("%s%s%s", lookup(var.custom_rules[count.index], "access", ""), lookup(var.custom_rules[count.index], "name", ""), lookup(var.custom_rules[count.index], "direction", ""))
